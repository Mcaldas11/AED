...
# path do ficheiro
ficheiro = ".\\ficha10\\presencas.txt"
...
# FRAME
    ...
    btnregistar = customtkinter.CTkButton(frameGerir, text ="Registar o Movimento \nem Ficheiro", fg_color="black", text_color="cyan",
                        width=250, height=170, command = lambda: registoMovimento(entryAluno, cb1, cb2))
...
def renderConsulta():
    ...
    img1 = customtkinter.CTkImage(Image.open(".\\ficha10\\lupa.png"), size=(64, 64))
...
def registoMovimento(entryAluno, cb1, cb2):
    numero = entryAluno.get().strip()
    movimento = None
    ...
    if not numero or not movimento:
        CTkMessagebox.CTkMessagebox(title="Erro", message="Número e Tipo de Movimento são obrigatórios!", icon="warning")
        return
    ...
    for linha in (listaMov):
        ...
        if campos[0] == numero and campos[1] == data:
            ultimo_mov = campos[3]
            break

    if ultimo_mov == movimento:
        CTkMessagebox.CTkMessagebox(title="Erro", message=f"O último movimento registado foi '{ultimo_mov}'. O próximo deverá ser diferente.", icon="warning")
        return
    listaMov.append(f"{numero};{data};{hora};{movimento}\n")
    
    fileAcessos = open(ficheiro, "w", encoding="utf-8")
    fileAcessos.writelines(listaMov)
    fileAcessos.close()

    CTkMessagebox.CTkMessagebox(title="Sucesso", message="Movimento registado com sucesso!", icon="check")
...
imageIco1 = customtkinter.CTkImage(Image.open(".\\ficha10\\icoOp1.png"), size=(64, 64))
...
imageIco2 = customtkinter.CTkImage(Image.open(".\\ficha10\\icoOp2.png"), size=(64, 64))
...
imageIco4 = customtkinter.CTkImage(Image.open(".\\ficha10\\icoOp4.png"), size=(64, 64))
...
imageLabel = customtkinter.CTkImage(Image.open(".\\ficha10\\presencas.png"), size=(750, 500))
