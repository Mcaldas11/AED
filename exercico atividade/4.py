...
def consultarTree(tree, mes, filter, label1, label2):
    ...
    Arg: tree, mes selecionado e filtros selecionados
    """
    mes = mes.replace("\n", "")  # formatar o mes
    ...
    for linha in lista:  # adiciona as linhas a tree
        campos = linha.split(";")
        if filter.get() + "\n" == campos[2] or filter.get() == "Todas":  # filtra segundo os botoes selecionados
            tree.insert("", "end", values=(campos[0], campos[1], campos[2]))
            despesasN += 1
            despesasV += int(campos[1])
    
    # Atualiza as labels diretamente
    label1.configure(text=despesasN)
    label2.configure(text=despesasV)
